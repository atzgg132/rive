...
function ContractPdfDocument({
  ...
  return (
    <Document title={content.title} author="rive." subject="Freelance services Agreement">
      <Page size="A4" style={styles.page} wrap>
        <View style={styles.header}>
          <Text style={styles.brand}>RIVE. AGREEMENT RECORD</Text>
          <Text style={styles.title}>{content.title}</Text>
          <Text style={styles.meta}>Status: {status === "executed" ? "accepted" : status} · Governing law: {effectiveGoverningLaw}{effectiveJurisdiction ? ` · Jurisdiction: ${effectiveJurisdiction}` : ""}</Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          {content.paymentPlan.items.length ? (
            ...
            <Text style={styles.body}>No automatic payment plan was attached to this Agreement.</Text>
          )}
        </View>

        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Recorded acceptance</Text>
          {signatures.length ? signatures.map((signature) => (
            <View key={`${signature.role}-${signature.email}`} style={{ marginBottom: 7 }}>
              ...
              <Text style={styles.meta}>{signature.email} · accepted by typed name {new Date(signature.signedAt).toISOString()}</Text>
              <Text style={styles.evidence}>Acceptance record: {signature.id} · consent {signature.consentTextVersion}</Text>
          ...
          )) : <Text style={styles.body}>No recorded acceptance has been added to this version.</Text>}
        </View>

        <View style={styles.footer}>
          <Text>Document hash: {documentHash}</Text>
          <Text>Evidence hash: {evidenceHash}</Text>
          <Text>Acceptance method: recorded acceptance (adapter: {provider})</Text>
          <Text>Accepted at: {executedAt || "not accepted"}</Text>
          <Text>This record is generated by rive. The parties remain responsible for reviewing the Agreement, party authority, identity/authentication, applicable law, and any exclusions or formalities that apply to their transaction.</Text>
        </View>
      </Page>
    </Document>
  );
...
function formatTrigger(item: ContractContent["paymentPlan"]["items"][number]): string {
  if (item.triggerType === "on_signing") return "On recorded acceptance";
