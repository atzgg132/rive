...
export function CalendarConnectionsPanel({ onChange }: { onChange?: () => void } = {}) {
  ...
  async function disconnectConnection(connection: Connection) {
    ...
    const confirmed = await confirm({
      ...
      description: "Calendars and events imported from it are removed from rive. Your Google Calendar itself is unchanged.",
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="rounded-none border border-border p-4">
        ...
        {!feedUrl ? <Button variant="outline" onClick={() => void createAppleFeed()} className="mt-4 w-full text-xs font-bold">Create private Apple Calendar feed</Button> : <div className="mt-4"><div className="flex gap-2"><Input readOnly value={feedUrl} className="min-w-0 flex-1 font-mono text-xs tabular-nums" /><Button variant="outline" size="sm" onClick={() => void copyFeed()} className="px-3" aria-label={copied ? "Feed link copied" : "Copy feed link"}>{copied ? <Check className="h-4 w-4 text-success" /> : <Copy className="h-4 w-4 text-muted-foreground" />}</Button><a href={feedUrl} aria-label="Open feed in Calendar" className="grid place-items-center rounded-none bg-foreground px-3 text-background"><ExternalLink className="h-4 w-4" /></a></div><div className="mt-2 flex items-start justify-between gap-3"><p className="text-xs leading-4 text-muted-foreground">Treat this URL like a password. Regenerating it revokes the previous feed.</p><Button onClick={() => void revokeAppleFeed()} className="shrink-0 text-xs font-bold text-destructive hover:underline">Revoke feed</Button></div></div>}
      </section>
