...
export function StartEngagementComposer({ entryPoint, currency, agreementsAvailable, clients: suppliedClients, inquiry, onCreated }: Props) {
  ...
  return (
    <section aria-labelledby="start-engagement-heading" className="overflow-hidden rounded-none border border-border bg-card">
      <div className="border-b border-border bg-primary/[0.035] px-5 py-5 sm:px-7">
        ...
        <ol className="mt-5 grid grid-cols-3 gap-2" aria-label="Engagement progress">
          {STEPS.map((item, index) => {
            ...
            return (
              <li key={item.id} className={`flex min-w-0 items-center gap-2 rounded-none border px-3 py-2 text-xs font-bold ${index === step ? "border-primary/30 bg-primary/10 text-primary" : index < step ? "border-success/25 bg-success/10 text-success" : "border-border bg-background text-muted-foreground"}`}>
      ...
      <div className="p-5 sm:p-7">
        {step === 0 ? (
          <div className="space-y-5">
            {inquiry ? (
              <div className="rounded-none border border-primary/20 bg-primary/[0.035] p-4">
                <Kicker>Converted Client</Kicker>
              ...
              <div className="grid grid-cols-2 gap-2 rounded-none bg-muted p-1" role="group" aria-label="Client source">
            ...
            <div className="rounded-none border border-border bg-muted/40 p-4 text-sm text-muted-foreground">
        ...
        {step === 1 ? (
          <div className="grid gap-4 sm:grid-cols-2">
            {inquiry ? <div className="sm:col-span-2 rounded-none border border-border bg-muted/[0.35] p-4"><Kicker tone="muted" dot={false}>Visitor message · read-only</Kicker><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-foreground">{inquiry.message}</p></div> : null}
            ...
            <div className="sm:col-span-2 flex items-start gap-3 rounded-none border border-primary/15 bg-primary/[0.035] p-4 text-sm">
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className={`mt-3 grid gap-3 ${agreementsAvailable ? "sm:grid-cols-2" : ""}`}>
                <button type="button" onClick={() => setScopeMode("project")} className={`rounded-none border p-4 text-left transition ${scopeMode === "project" ? "border-primary bg-primary/[0.05] ring-2 ring-primary/10" : "border-border hover:border-primary/30"}`}>
                ...
                {agreementsAvailable ? <button type="button" onClick={() => setScopeMode("agreement")} className={`rounded-none border p-4 text-left transition ${scopeMode === "agreement" ? "border-primary bg-primary/[0.05] ring-2 ring-primary/10" : "border-border hover:border-primary/30"}`}>
            ...
            <div className="rounded-none inverse-block p-5">
              <p className="text-xs font-black uppercase tracking-[0.16em]">Rive will create</p>
              <ul className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
                <li className="flex items-center gap-2"><Check className="h-4 w-4" /> {clientMode === "existing" ? selectedClient?.name || "Selected client" : clientName || "New client"}</li>
                <li className="flex items-center gap-2"><Check className="h-4 w-4" /> {projectTitle || "Project"}</li>
