  ...
  return (
    <section className={sectionClass}>
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h2 className="font-bold text-foreground">Testimonials</h2>
          <p className="mt-1 max-w-2xl text-xs leading-5 text-muted-foreground">Add historical client quotes you have permission to share. These are imported testimonials, not Rive-verified reviews.</p>
        </div>
        <Button type="button" onClick={() => onUpdateContent({ testimonials: [...content.testimonials, createStudioTestimonial()] })} className="inline-flex shrink-0 items-center justify-center gap-1 rounded-none bg-primary/10 px-2.5 py-2 text-xs font-bold text-primary"><Plus className="h-3.5 w-3.5" /> Add testimonial</Button>
      </div>
      {content.testimonials.length === 0 ? (
        <div className="rounded-none border border-dashed border-border px-4 py-8 text-center text-sm text-muted-foreground">No testimonials yet. Add one when you have a past client quote ready.</div>
      ) : (
        <div className="flex flex-col gap-4">
          {content.testimonials.map((testimonial) => (
            <div key={testimonial.id} className="rounded-none border border-border p-4">
              <div className="mb-3 flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-[0.12em] text-muted-foreground">Historical testimonial</span>
                <Button type="button" title="Remove testimonial" onClick={() => onUpdateContent({ testimonials: content.testimonials.filter((item) => item.id !== testimonial.id) })} className="rounded-none p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive"><Trash2 className="h-4 w-4" /></Button>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="flex flex-col gap-2 sm:col-span-2"><span className={labelClass}>Quote <span className="text-primary">Required</span></span><Textarea className={inputClass} rows={4} value={testimonial.quote} placeholder="What did the client say about working with you?" onChange={(event) => onUpdateContent({ testimonials: content.testimonials.map((item) => item.id === testimonial.id ? { ...item, quote: event.target.value } : item) })} /></label>
                <label className="flex flex-col gap-2"><span className={labelClass}>Client name <span className="text-primary">Required</span></span><Input className={inputClass} value={testimonial.name} placeholder="Jordan Lee" onChange={(event) => onUpdateContent({ testimonials: content.testimonials.map((item) => item.id === testimonial.id ? { ...item, name: event.target.value } : item) })} /></label>
