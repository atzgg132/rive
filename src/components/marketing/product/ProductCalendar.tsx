...
export function ProductCalendar({ month, days, events }: ProductCalendarProps) {
  return (
    <ProductFrame
      ...
      eyebrow="Calendar"
      toolbar={
        <span className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[0.55rem] font-bold">
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        <div className="grid grid-cols-5 border-b border-slate-200 bg-slate-50">
          {days.map((day, index) => (
            <div
              key={day}
              className={`px-2 py-2 text-center font-mono text-[0.48rem] font-bold uppercase tracking-[0.08em] ${index ? "border-l border-slate-200" : ""} ${index === 2 ? "text-blue-700" : "text-slate-600"}`}
            >
              {day}
            </div>
          ))}
        ...
        <div className="relative grid h-44 grid-cols-5 lg:h-64">
          ...
          {events.map((event, index) => (
            <div
              key={event.label}
              className={`marketing-mock-in absolute z-10 overflow-hidden truncate whitespace-nowrap rounded-md border px-2 py-1 text-[0.5rem] font-bold shadow-sm ${tones[event.tone]}`}
              style={{
                left: `calc(${event.day * 20}% + 4px)`,
                width: "calc(20% - 8px)",
                top: `${event.start * 12.5 + 1}%`,
                height: `${event.span * 12.5 - 2}%`,
                animationDelay: `${0.18 + index * 0.16}s`,
              }}
            >
              <CalendarDays className="mb-1 h-2.5 w-2.5" />
              {event.label}
            </div>
          ))}
          <div className="marketing-calendar-now pointer-events-none absolute inset-x-0 z-20 flex items-center">
