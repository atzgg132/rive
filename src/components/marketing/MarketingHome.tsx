  ...
  return (
    <>
      <SpotlightCursor>
        ...
        >
          ...
          <div className="relative z-10 mx-auto flex max-w-[76rem] flex-col items-center px-5 text-center sm:px-8">
            ...
            <h1 className="mt-5 text-[clamp(3.25rem,7.5vw,6.5rem)] font-black leading-[0.94] tracking-[-0.05em] text-white sm:mt-6">
            ...
            <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-slate-300 sm:mt-6 sm:text-xl sm:leading-9">{homeContent.hero.body}</p>
            <div className="mt-6 flex flex-col justify-center gap-3 sm:mt-8 sm:flex-row sm:items-center">
              ...
              <SmoothAnchor
                ...
                className="inline-flex min-h-11 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] px-5 text-sm font-bold text-slate-200 transition duration-200 ease-rive-out hover:-translate-y-0.5 hover:border-blue-300/25 hover:bg-white/[0.07]"
              >
            ...
            <div className="mt-5 flex flex-wrap justify-center gap-x-5 gap-y-2 sm:mt-8 sm:gap-x-6 sm:gap-y-3">
              {homeContent.hero.proof.map((item) => (
                <span key={item} className="inline-flex items-center gap-2 font-mono text-[0.64rem] font-semibold uppercase tracking-[0.13em] text-slate-400">
      ...
      <SectionShell id="import">
        <div className="grid gap-12 lg:grid-cols-2 lg:items-center lg:gap-20">
          <RevealOnScroll>
            ...
            <h2 className="mt-6 text-4xl font-black leading-[1.02] tracking-[-0.05em] text-white sm:text-6xl">{homeContent.import.title}</h2>
      ...
      <SectionShell id="agreements" className="bg-white/[0.012]">
        <div data-testid="marketing-agreements-section" className="grid gap-12 lg:grid-cols-2 lg:items-center lg:gap-20">
          <RevealOnScroll>
            ...
            <h2 className="mt-6 text-4xl font-black leading-[1.02] tracking-[-0.05em] text-white sm:text-6xl">{homeContent.agreement.title}</h2>
            <p className="mt-6 text-base leading-8 text-slate-400">{homeContent.agreement.body}</p>
            <p className="mt-8 font-mono text-xs font-semibold uppercase tracking-[0.16em] text-blue-300"><span>{homeContent.agreement.tags[0]}</span><span aria-hidden="true"> · </span><span>{homeContent.agreement.tags[1]}</span></p>
          </RevealOnScroll>
        ...
        <div data-testid="remit-section" className="grid gap-12 lg:grid-cols-[.9fr_1.1fr] lg:items-start lg:gap-16">
          ...
          <GlassPanel tier={3} className="min-w-0 p-6 sm:p-8 lg:min-w-[28rem]">
            <div data-testid="remit-calculator">
              <div className="flex items-center justify-between"><p className="font-mono text-[0.64rem] font-semibold uppercase tracking-[0.16em] text-blue-300">{homeContent.remit.ledger.label}</p><Braces className="h-4 w-4 text-slate-400" /></div>
              <div className="mt-6 overflow-hidden rounded-xl border border-white/[0.08]">
                <div className="hidden grid-cols-[1.15fr_1fr_1fr_1fr] gap-3 border-b border-white/[0.07] bg-white/[0.02] px-4 py-2.5 sm:grid">
                  {["Record", "Native amount", "Rate · date", "Display"].map((column, index) => (
                    <span key={column} className={index === 0 ? "font-mono text-[0.54rem] font-semibold uppercase tracking-[0.12em] text-slate-500" : "text-right font-mono text-[0.54rem] font-semibold uppercase tracking-[0.12em] text-slate-500"}>{column}</span>
                  ))}
                </div>
                {homeContent.remit.ledger.rows.map((row) => (
                  <div key={row.record} className="grid grid-cols-[1fr_auto] gap-x-3 gap-y-1.5 border-b border-white/[0.06] px-4 py-3 last:border-b-0 sm:grid-cols-[1.15fr_1fr_1fr_1fr] sm:items-baseline">
                    <span>
                      <span className="block text-sm font-bold tracking-[-0.02em] text-slate-100">{row.record}</span>
                      <span className="mt-0.5 block text-[0.66rem] text-slate-500">{row.kind}</span>
                    </span>
                    <span className="text-right font-mono text-xs font-semibold tabular-nums text-slate-200">{row.native}</span>
                    <span className="col-start-1 font-mono text-[0.62rem] tabular-nums text-slate-500 sm:col-start-3 sm:text-right">{row.rate}</span>
                    <span className="col-start-2 text-right font-mono text-xs font-semibold tabular-nums text-blue-200 sm:col-start-4">{row.display}</span>
                  </div>
                ))}
              </div>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                {homeContent.remit.ledger.totals.map((total, index) => (
                  <div key={total.label} className={index === 0 ? "rounded-xl border border-blue-300/15 bg-[#101a2c] p-4" : "rounded-xl border border-white/[0.08] bg-[#101722] p-4"}>
                    <p className={index === 0 ? "font-mono text-[0.56rem] uppercase tracking-[0.12em] text-blue-300" : "font-mono text-[0.56rem] uppercase tracking-[0.12em] text-slate-400"}>{total.label}</p>
                    <p className="mt-2 text-2xl font-black tabular-nums text-white">{total.value}</p>
                  </div>
                ))}
              </div>
              <div className="mt-4 flex items-center justify-between gap-4 rounded-xl border border-white/[0.07] p-4 text-sm"><span className="text-slate-400">Status</span><span className="font-mono text-[0.6rem] font-semibold uppercase tracking-[0.1em] text-amber-300">{homeContent.remit.ledger.note}</span></div>
            </div>
          </GlassPanel>
        </div>
      </SectionShell>
      ...
      <SectionShell id="portfolio" className="bg-white/[0.012]">
        <div className="grid gap-12 lg:grid-cols-2 lg:items-center lg:gap-20">
          <RevealOnScroll><GlowingBadge>{homeContent.portfolio.eyebrow}</GlowingBadge><h2 className="mt-6 text-4xl font-black leading-[1.02] tracking-[-0.05em] text-white sm:text-6xl">{homeContent.portfolio.title}</h2><p className="mt-6 text-base leading-8 text-slate-400">{homeContent.portfolio.body}</p></RevealOnScroll>
      ...
      <SectionShell id="remit-transfers" className="bg-white/[0.012]">
          ...
          <div data-testid="remit-next-section" className="grid gap-10 lg:grid-cols-[.95fr_1.05fr] lg:items-center lg:gap-16">
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className="font-mono text-[0.62rem] font-semibold uppercase tracking-[0.14em] text-amber-300">{homeContent.remitNext.status}</span>
              </div>
              <h2 className="mt-6 text-3xl font-black leading-[1.04] tracking-[-0.045em] text-white sm:text-5xl">{homeContent.remitNext.title}</h2>
              <p className="mt-6 max-w-2xl text-base leading-8 text-slate-400">{homeContent.remitNext.body}</p>
              <dl className="mt-8 divide-y divide-white/[0.07] border-y border-white/[0.07]">
                {homeContent.remitNext.promises.map((promise) => (
                  <div key={promise.label} className="py-3.5">
                    <dt className="text-sm font-bold tracking-[-0.02em] text-white">{promise.label}</dt>
                    <dd className="mt-1 text-sm leading-6 text-slate-500">{promise.sub}</dd>
                  </div>
                ))}
              </dl>
