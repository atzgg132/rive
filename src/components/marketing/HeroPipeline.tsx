...
export function HeroPipeline() {
  ...
  return (
    <div data-testid="hero-pipeline" className="relative mx-auto w-full max-w-5xl min-w-0 overflow-x-hidden px-1 animate-hero-preview-in sm:px-2">
          ...
          {stages.map((stage, index) => {
            ...
            return (
              <button
                ...
                className="marketing-focus group relative flex min-h-11 min-w-0 flex-col items-center gap-1 rounded-xl px-0.5 py-1 text-center transition duration-300 ease-rive-out sm:gap-1.5"
              >
                <span
                  className={`relative z-10 grid h-6 w-6 shrink-0 place-items-center rounded-lg ring-1 transition duration-300 ease-rive-out sm:h-7 sm:w-7 ${
                    selected
      ...
      <div key={active.id} className="mt-1.5 text-center motion-safe:animate-hero-detail-in sm:mt-2">
        <p className="mx-auto max-w-2xl text-balance text-[0.78rem] font-semibold leading-5 text-muted-foreground sm:text-[0.8rem] sm:leading-5">
          {active.detail}
        </p>
        <p className="mt-1 flex items-center justify-center gap-1.5 font-mono text-[0.56rem] font-semibold uppercase tracking-[0.14em] text-primary">
