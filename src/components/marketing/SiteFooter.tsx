...
export function SiteFooter() {
  return (
    <footer className="marketing-deferred-section relative overflow-x-clip border-t border-white/[0.07] bg-[#05070c] py-14 sm:py-20">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <div className="grid gap-12 lg:grid-cols-[1.1fr_2fr] lg:gap-20">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-10 sm:grid-cols-4">
            {footerNav.map((group) => (
              <div key={group.label}>
                <h2 className="font-mono text-[0.66rem] font-semibold uppercase tracking-[0.16em] text-slate-400">{group.label}</h2>
                <ul className="mt-4 grid gap-3">
                  {group.items.map((item) => (
                    <li key={item.href}>
                      <Link href={item.href} prefetch={false} className="marketing-focus inline-flex items-center gap-1 rounded text-sm font-semibold text-slate-300 transition hover:text-blue-300">{item.label}{item.href.startsWith("mailto:") ? <ArrowUpRight className="h-3 w-3" /> : null}</Link>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="flex flex-col gap-3 font-mono text-[0.62rem] uppercase tracking-[0.13em] text-slate-400 sm:flex-row sm:items-center sm:justify-between">
