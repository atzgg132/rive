...
export function AuthOverlay({
  ...
  return (
    ...
    >
      <BaseDialog.Portal>
        <BaseDialog.Backdrop
          className="auth-overlay-backdrop fixed inset-0 z-[80] min-h-dvh bg-black/40 backdrop-blur-md dark:bg-black/55"
        />
        <BaseDialog.Viewport className="fixed inset-0 z-[80] flex min-h-dvh items-end justify-center overflow-y-auto p-0 sm:items-center sm:p-6">
          <BaseDialog.Popup
            ...
            className="auth-overlay-panel relative w-full max-w-[26.5rem] max-h-[90dvh] overflow-y-auto rounded-t-[1.6rem] border border-[var(--stroke-hairline)] bg-[color-mix(in_srgb,var(--surface-raised)_92%,transparent)] p-6 pb-10 text-foreground shadow-overlay backdrop-blur-2xl sm:max-h-[min(100dvh-3rem,_42rem)] sm:rounded-[1.6rem] sm:p-8 sm:pb-8"
          >
            <BaseDialog.Close
              disabled={busy}
              className="absolute right-3 top-3 inline-flex h-9 w-9 appearance-none items-center justify-center rounded-lg border-0 bg-transparent text-muted-foreground shadow-none transition-colors duration-150 ease-rive-out hover:bg-[var(--surface-glass)] hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 disabled:opacity-40"
              aria-label="Close"
            >
              <X className="h-4 w-4" />
            </BaseDialog.Close>
...
export function AuthEnterVeil({ visible }: { visible: boolean }) {
  return (
    <div
      className="auth-enter-veil fixed inset-0 z-[90] bg-[rgb(var(--background))]"
