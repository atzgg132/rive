...
// auth. The page only ever talks to the reserved "session" API paths, so the
// raw token never has to live in the address bar or in page state.
const SIGN_API = "/api/public/contracts/sign/session";
const VOID_API = "/api/public/contracts/void/session";
...
export default function ContractSignPublicPage() {
  ...
  const load = async () => {
    try {
      const response = await fetch(SIGN_API, { cache: "no-store" });
  ...
  const decline = async () => {
    if (declining || declineReason.trim().length < 5) return;
    setDeclining(true);
    try {
      const response = await fetch(SIGN_API, {
        method: "POST",
