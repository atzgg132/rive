...
export async function PUT(req: NextRequest) {
  ...
  const profile = await prisma.invoiceProfile.upsert({
    ...
    create: { userId: session.userId, businessName: text(body.businessName, 180), contactName: text(body.contactName, 160), email, phone: text(body.phone, 80), address: text(body.address, 1_000), taxId: text(body.taxId, 120), logoUrl: text(body.logoUrl, 1_000), defaultCurrency, invoicePrefix, paymentInstructions: text(body.paymentInstructions, 2_000), defaultTerms: text(body.defaultTerms, 2_000) },
    update: { businessName: text(body.businessName, 180), contactName: text(body.contactName, 160), email, phone: text(body.phone, 80), address: text(body.address, 1_000), taxId: text(body.taxId, 120), logoUrl: text(body.logoUrl, 1_000), defaultCurrency, invoicePrefix, paymentInstructions: text(body.paymentInstructions, 2_000), defaultTerms: text(body.defaultTerms, 2_000) },
  });
  return NextResponse.json({ success: true, profile });
}
