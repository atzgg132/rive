...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await Promise.all([
      ...
      recordActivationEvent(session.userId, ACTIVATION_EVENTS.firstClientCreated, { source: eventSource }),
      recordActivationEvent(session.userId, ACTIVATION_EVENTS.firstProjectCreated, { source: eventSource }),
      ...(result.createdClient
        ? [recordProductEvent({ ...sharedEvent, eventName: PRODUCT_EVENTS.clientCreated, module: "clients", entityType: "client", entityId: result.records.clientId, dedupeKey: `engagement:${session.userId}:${eventRequestId}:client_created` })]
        : []),
      recordProductEvent({ ...sharedEvent, eventName: PRODUCT_EVENTS.projectCreated, module: "projects", entityType: "project", entityId: result.records.projectId, dedupeKey: `engagement:${session.userId}:${eventRequestId}:project_created` }),
      ...(result.records.invoiceId
        ? [recordProductEvent({ ...sharedEvent, eventName: PRODUCT_EVENTS.invoiceCreated, module: "invoices", entityType: "invoice", entityId: result.records.invoiceId, dedupeKey: `engagement:${session.userId}:${eventRequestId}:invoice_created` })]
        : []),
      recordProductEvent({
        ...sharedEvent,
        ...
        dedupeKey: `engagement:${session.userId}:${eventRequestId}:created`,
        properties: {
    ...
    if (input) {
      await recordProductEvent({
        ...
        module: "engagements",
        source: input.entryPoint === "inquiry" ? "portfolio_inquiry" : "engagement_flow",
