...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (action === "request") {
      ...
      if (clientSigner?.email) {
        await sendContractVoidRequestedEmail({ to: clientSigner.email, recipientName: clientSigner.name, contractTitle: contract.title, requesterName: contract.user.name || contract.user.email, note }).catch(() => undefined);
      }
    ...
    if (action === "confirm") {
      ...
      await prisma.$transaction(async (tx) => {
        const voided = await transitionContractStatus(tx, { where: { id, userId: session.userId, status: "executed" }, from: "executed", to: "void", data: { voidedAt: new Date(), voidConfirmNote: note } });
