...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (contract.status !== "executed") return NextResponse.json({ success: false, message: "Only executed contracts have a completed artifact." }, { status: 409 });
    const version = contract.versions[0];
    const artifact = contract.artifacts[0];
    if (!version || !artifact) return NextResponse.json({ success: false, message: "Completed artifact is not available yet." }, { status: 404 });
    ...
    const filename = `${contract.title.replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").slice(0, 80) || "contract"}-executed.pdf`;
