...
import { CONTRACT_MAX_COMMENT_LENGTH } from "@/utils/contracts";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!contract.versions[0]) return NextResponse.json({ success: false, message: "Contract version not found." }, { status: 409 });
