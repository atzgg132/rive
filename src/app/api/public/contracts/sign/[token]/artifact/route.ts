...
export async function GET(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  try {
    ...
    if (!link || link.type !== "sign" || link.revokedAt || link.expiresAt <= new Date() || !link.version || !link.signer) {
      return NextResponse.json({ success: false, message: "Artifact link not found or expired." }, { status: 404 });
    }
    if (link.contract.status !== "executed") return NextResponse.json({ success: false, message: "The contract is not executed yet." }, { status: 409 });
    ...
    if (!artifact) return NextResponse.json({ success: false, message: "Completed artifact is not available yet." }, { status: 404 });
    ...
    console.error("Public contract artifact error:", error);
