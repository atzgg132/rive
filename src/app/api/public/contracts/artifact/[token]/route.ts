  ...
  try {
    ...
    const problem = !link
      ...
      : link.type !== "artifact"
        ? "Accepted Agreement link not found or expired."
        : link.revokedAt
          ? "Accepted Agreement link has been revoked."
          : link.expiresAt <= new Date()
            ? "Accepted Agreement link has expired."
            : !link.version
              ? "Accepted Agreement link is incomplete."
              : link.contract.status !== "executed"
                ? "Accepted Agreement record is not ready."
                : null;
    if (problem) {
    ...
    const signatures = await prisma.contractSignature.findMany({ where: { contractId: link!.contractId, versionId: link!.version!.id }, orderBy: { signedAt: "asc" }, take: 10, select: { id: true, signerRole: true, signerName: true, signerEmail: true, consentTextVersion: true, providerEventId: true, ipHash: true, userAgentHash: true, signedAt: true } });
