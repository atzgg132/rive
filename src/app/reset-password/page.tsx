...
export default function ResetPasswordPage() {
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#F5F8FC] px-4 py-12 dark:bg-[#0B1120]">
      ...
      <section className="z-10 w-full max-w-md">
        ...
        <div className="glass rounded-2xl border border-[#E2EAF4] bg-white/85 p-8 shadow-[0_8px_30px_rgb(12,30,54,0.04)] dark:border-slate-800 dark:bg-slate-900/85">
          {success ? (
            <div className="text-center">
              ...
              <Link href="/login" className="mt-6 inline-flex rounded-xl bg-[#1D4ED8] px-6 py-3 text-sm font-bold text-white">Continue to login</Link>
            </div>
          ) : (
            <>
              ...
              {!token ? (
                ...
                <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                  ...
                  <button disabled={loading} className="flex w-full items-center justify-center rounded-xl bg-[#1D4ED8] py-3 text-sm font-bold text-white disabled:opacity-70">{loading ? <Loader2 className="h-5 w-5 animate-spin" /> : "Update password"}</button>
                </form>
              )}
            </>
          )}
