...
import { Badge, Button, Input, Kicker, Select, Tabs } from "@/components/ui";
...
function Metric({ label, value, detail, tone = "blue", footer }: { label: string; value: string | number; detail: string; tone?: "blue" | "green" | "amber" | "purple" | "red"; footer?: ReactNode }) {
  ...
  return <div className="rounded-none border border-border bg-card p-5"><div className={`mb-5 grid h-10 w-10 place-items-center rounded-none border ${colors[tone]}`}><Activity className="h-5 w-5" /></div><p className="font-mono text-3xl font-bold tracking-tight tabular-nums text-card-foreground">{value}</p><p className="mt-1 text-sm font-semibold text-card-foreground">{label}</p><p className="mt-1 text-xs text-muted-foreground">{detail}</p>{footer ? <div className="mt-3 border-t border-border pt-3">{footer}</div> : null}</div>;
}
...
function Reliability({ funnel, retry, loading, error }: { funnel: Funnel | null; retry: () => void; loading: boolean; error: string }) {
  ...
  return <div className="space-y-6"><div><p className="text-sm font-semibold text-primary">Reliability</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-foreground">Is the product actually working</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Delivery and instrumentation health. If email is failing here, signups look slow on every other tab for a reason that has nothing to do with demand.</p></div>{error ? <LoadError message={error} onRetry={retry} loading={loading} /> : null}<div className="grid gap-4 sm:grid-cols-3"><Metric label="Failed emails / 24h" value={funnel.reliability.failedEmails24h} detail={funnel.reliability.failedEmails24h > 0 ? "Password resets and verification links are affected" : "Delivery is healthy"} tone={funnel.reliability.failedEmails24h > 0 ? "red" : "green"} /><Metric label="Queued email jobs" value={funnel.reliability.queuedEmails} detail={funnel.reliability.queuedEmails > 0 ? "Waiting in the outbox to be retried" : "Outbox is empty"} tone={funnel.reliability.queuedEmails > 0 ? "amber" : "blue"} /><Metric label="Product events / 24h" value={funnel.reliability.productEvents24h} detail={funnel.quality.eventLagMinutes === null ? "No events recorded yet" : `Most recent ${funnel.quality.eventLagMinutes}m ago`} tone={funnel.reliability.productEvents24h === 0 ? "amber" : "blue"} /></div><Panel title="Action queue" eyebrow={quality.alerts.length ? `${quality.alerts.length} active signal${quality.alerts.length === 1 ? "" : "s"}` : "No active threshold breaches"}>{quality.alerts.length ? <div className="space-y-3">{quality.alerts.map((item) => <div key={item.fingerprint} className={`rounded-none border p-4 ${item.severity === "critical" ? "border-destructive/25 bg-destructive/10 text-destructive" : "border-warning/25 bg-warning/10 text-warning"}`}><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-semibold">{item.title}</p><Badge variant={item.severity === "critical" ? "destructive" : "warning"} className="uppercase tracking-wide">{item.severity}</Badge></div><p className="mt-2 text-sm opacity-80">{item.detail}</p><p className="mt-2 text-xs font-semibold opacity-90">Next: {item.action}</p></div>)}</div> : <p className="text-sm text-muted-foreground">The scheduled funnel-quality check has no active threshold breaches.</p>}</Panel><Panel title="Cohort-quality monitoring" eyebrow={`Event envelope schema v${quality.schemaVersion}`}><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><div className="rounded-none bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Contract rejects / 24h</p><p className="mt-1 font-mono text-2xl font-bold tabular-nums">{quality.contractRejections24h}</p></div><div className="rounded-none bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Unknown event names / 24h</p><p className="mt-1 font-mono text-2xl font-bold tabular-nums">{quality.unknownEventNames24h}</p></div><div className="rounded-none bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Uncaptured signup source</p><p className="mt-1 font-mono text-2xl font-bold tabular-nums">{quality.uncapturedSignups} <span className="text-sm font-normal text-muted-foreground">({rate(quality.uncapturedSignupRate)})</span></p></div><div className="rounded-none bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Missing event identity / 24h</p><p className="mt-1 font-mono text-2xl font-bold tabular-nums">{quality.missingIdentityEvents24h}</p></div><div className="rounded-none bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Missing data origin / 24h</p><p className="mt-1 font-mono text-2xl font-bold tabular-nums">{quality.missingDataOriginEvents24h}</p></div><div className="rounded-none bg-muted/50 p-4"><p className="text-xs text-muted-foreground">Event freshness</p><p className="mt-1 font-mono text-2xl font-bold tabular-nums">{quality.eventLagMinutes === null ? "No events" : `${quality.eventLagMinutes}m`}</p></div></div><p className="mt-4 text-xs text-muted-foreground">Unknown-origin business records: {quality.unknownOriginRecords}. Latest event: {quality.latestEventAt ? ago(quality.latestEventAt) : "not recorded"}.</p></Panel><Panel title="When not to trust these numbers"><ul className="grid gap-3 text-sm text-muted-foreground md:grid-cols-2"><li className="rounded-none bg-muted/50 p-4"><span className="font-semibold text-card-foreground">Event freshness is stale.</span> Analytics writes never block a business action, so a broken pipeline shows up as flat metrics rather than errors. Check freshness above before reading a drop as real.</li><li className="rounded-none bg-muted/50 p-4"><span className="font-semibold text-card-foreground">Contract rejects are non-zero.</span> Rejected events are discarded, so the funnel undercounts by roughly that amount.</li><li className="rounded-none bg-muted/50 p-4"><span className="font-semibold text-card-foreground">Uncaptured signup source is high.</span> Those users are counted in totals but cannot be attributed, so the source breakdown understates every channel.</li><li className="rounded-none bg-muted/50 p-4"><span className="font-semibold text-card-foreground">Emails are failing.</span> Unverified accounts stall before qualifying, which depresses every downstream rate for a delivery reason, not a product one.</li></ul></Panel></div>;
}
...
function LegacyTab() {
  ...
  useEffect(() => { const timer = window.setTimeout(() => void load(), 0); return () => window.clearTimeout(timer); }, [load]);
  return <div className="space-y-6"><div><p className="text-sm font-semibold text-primary">Legacy archive</p><h1 className="mt-1 text-3xl font-bold tracking-tight text-foreground">Pre-launch waitlist</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Read-only. Nobody new enters this list; it is kept so old signups stay traceable and old links keep resolving. These entries are excluded from every metric on the other tabs.</p></div><Panel title="Archived entries">{error ? <LoadError message={error} onRetry={load} loading={loading} /> : loading ? <Loading label="Loading legacy archive" /> : <div className="overflow-x-auto"><div className="workspace-table"><table className="w-full text-left text-sm"><thead className="text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="pb-3">Email</th><th className="pb-3">Original source</th><th className="pb-3">Status</th><th className="pb-3">Created</th></tr></thead><tbody className="divide-y divide-border">{items.map((item) => <tr key={item.id}><td className="py-3">{item.email}</td><td className="py-3">{item.type}</td><td className="py-3">{item.registered ? "Registered" : item.status}</td><td className="py-3 font-mono tabular-nums">{ago(item.created_at)}</td></tr>)}</tbody></table></div>{!items.length ? <Empty text="No legacy entries." /> : null}</div>}</Panel></div>;
}
...
function Dashboard({ onLogout, onSessionExpired }: { onLogout: () => void; onSessionExpired: () => void }) {
  ...
  return <div className="min-h-screen bg-background"><header className="sticky top-0 z-20 border-b border-border bg-card/95 px-4 py-3 backdrop-blur sm:px-8"><div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4"><div className="flex items-center gap-5"><RiveLogo height={28} /><span className="hidden h-5 w-px bg-border sm:block" /><span className="hidden text-sm font-semibold text-muted-foreground sm:block">Admin workspace</span></div><div className="flex items-center gap-2"><span className="hidden rounded-full bg-success/10 px-3 py-1 text-xs font-semibold text-success sm:block">Open beta</span><Button type="button" variant="ghost" size="sm" onClick={onLogout} className="gap-2"><LogOut className="h-4 w-4" /> Sign out</Button></div></div></header><div className="mx-auto flex max-w-[1500px] flex-col gap-6 px-4 py-6 sm:px-8 lg:flex-row"><aside className="lg:w-52 lg:shrink-0"><Tabs options={tabs.map(({ id, label, icon: Icon }) => ({ id, label: (<span className="inline-flex items-center gap-2"><Icon className="h-4 w-4" />{label}</span>) }))} value={tab} onChange={(id) => setTab(id as Tab)} role="navigation" aria-label="Admin sections" className="overflow-x-auto" /></aside><main className="min-w-0 flex-1">{content}</main></div></div>;
}
