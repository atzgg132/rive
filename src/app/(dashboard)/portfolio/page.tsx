      ...
      {tab === "edit" && <div data-portfolio-editor-shell className="grid min-h-0 overflow-hidden rounded-2xl border border-border bg-card shadow-card lg:grid-cols-[210px_minmax(0,1fr)]">
        ...
        <div className="portfolio-editor-panels flex min-w-0 flex-col gap-5 p-5 sm:p-7">
          ...
          <section className={`rounded-2xl border border-border bg-white p-6 dark:border-slate-800 dark:bg-slate-900 ${editorSection === "work" ? "" : "hidden"}`}>
            ...
            <div className="flex flex-col gap-4">{content.projects.map((project, index) => <PortfolioProjectEditor key={project.id} project={project} index={index} practices={content.practices} onChange={(projectUpdate) => updateContent({ projects: content.projects.map((item) => item.id === project.id ? { ...item, ...projectUpdate } : item) })} onDelete={() => updateContent({ projects: content.projects.filter((item) => item.id !== project.id) })} onUploadCover={(file) => { void handleImageUpload(project.id, file); }} />)}</div>
          </section>

          <section className={`rounded-2xl border border-border bg-white p-6 dark:border-slate-800 dark:bg-slate-900 ${editorSection === "practices" ? "" : "hidden"}`}>
            <PortfolioPracticeEditor content={content} slug={slug} onChange={updateContent} />
          </section>
