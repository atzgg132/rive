...
export default function RevenuePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [invoiceResponse, summaryResponse] = await Promise.all([
        fetch(`/api/workflow/invoices?search=${encodeURIComponent(debouncedSearch)}&status=${encodeURIComponent(status)}&page=${page}&pageSize=${pageSize}${relatedQuery ? `&${relatedQuery}` : ""}`, { cache: "no-store" }),
  ...
  useEffect(() => { void load(); }, [debouncedSearch, status, page, pageSize]);
  ...
  return (
    <div className="workspace-page min-h-[calc(100vh-8rem)] space-y-7 animate-fade-in">
      ...
      <section className="rounded-2xl border border-border bg-card shadow-sm">
        ...
        {!loading && invoices.length > 0 && pagination ? <PaginationControls pagination={pagination} loading={loading} label="invoices" onPageChange={setPage} onPageSizeChange={(value) => { setPageSize(value); setPage(1); }} /> : null}
      </section>
