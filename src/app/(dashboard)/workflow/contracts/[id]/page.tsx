...
const statusMeta: Record<string, { label: string; description: string; badge: "default" | "secondary" | "outline" | "success" | "warning" | "destructive" }> = {
  ...
  ready_to_sign: { label: "Ready to sign", description: "This exact version is finalized and locked.", badge: "default" },
  starting: { label: "Preparing", description: "The signing request is being prepared.", badge: "warning" },
  signing: { label: "Signing", description: "Signatures are collected client first, owner second.", badge: "default" },
  executed: { label: "Executed", description: "Both signatures are recorded and payment triggers are active.", badge: "success" },
  declined: { label: "Changes requested", description: "A signer declined and explained what must change.", badge: "destructive" },
  expired: { label: "Expired", description: "Issue a fresh review or signing request.", badge: "warning" },
...
export default function ContractDetailPage() {
  ...
  const load = async (preserveEditor = false) => {
    ...
    try {
      ...
      if (!response.ok || !payload.success) throw new Error(payload.message || "Unable to load contract.");
  ...
  if (loading) return <div className="flex min-h-64 items-center justify-center text-sm text-muted-foreground"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Loading contract…</div>;
  if (loadError || !contract) return <EmptyState icon={<AlertTriangle className="h-5 w-5" />} title="Contract unavailable" description={loadError || "The contract was not found."} action={<Link href="/workflow/contracts" className={buttonVariants({ variant: "outline" })}>Back to contracts</Link>} />;
  ...
  const status = statusMeta[contract.status] || { label: contract.status, description: "Open contract record.", badge: "outline" as const };
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-6 animate-fade-in">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link href="/workflow/contracts" className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" /> Contracts</Link>
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {contract.status === "executed" ? <Button onClick={() => window.open(`/api/workflow/contracts/${id}/artifact`, "_blank")}><Download className="h-4 w-4" /> Download executed PDF</Button> : null}
      ...
      {reviewUrl ? <LinkPanel label="Client review link — comments only, not a signature request" url={reviewUrl} onCopy={copy} /> : null}
      {clientSignUrl ? <LinkPanel label="Client signing link — send only to the named client" url={clientSignUrl} onCopy={copy} /> : null}
      {ownerSignUrl ? <LinkPanel label="Owner signing link — use after the client signs" url={ownerSignUrl} onCopy={copy} /> : null}
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)]">
        <Card>
          <CardHeader><CardTitle>Contract terms</CardTitle><CardDescription>Immutable version {version?.version} · hash <span className="break-all font-mono text-[10px]">{version?.content_hash}</span></CardDescription></CardHeader>
        ...
        <aside className="flex flex-col gap-6">
          <Card>
            <CardHeader><CardTitle>Signing parties</CardTitle><CardDescription>Rive is not a party or signer.</CardDescription></CardHeader>
            <CardContent className="flex flex-col gap-3 pt-0 sm:pt-0">
              {contract.signers.map((signer) => (
                <div key={signer.id} className="rounded-xl border border-border p-3">
                  <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold">{signer.name}</p><p className="mt-0.5 text-xs text-muted-foreground">{signer.role} · {signer.email}</p></div><Badge variant={signer.status === "signed" ? "success" : signer.status === "declined" ? "destructive" : "outline"}>{signer.status}</Badge></div>
                  {signer.signatures.map((signature) => <p key={signature.signedAt} className="mt-2 text-[11px] leading-4 text-muted-foreground">Signed {new Date(signature.signedAt).toLocaleString()} · consent {signature.consentTextVersion}</p>)}
              ...
              {contract.status === "declined" && hasSignature ? <Alert variant="warning" className="text-xs"><AlertTriangle className="h-4 w-4" /><div><p className="font-bold">A signature already exists on this version</p><p className="mt-1 text-muted-foreground">Use Edit draft to save the requested changes as a new immutable version. The partial signature remains attached only to the declined version.</p></div></Alert> : null}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="flex flex-col gap-4 pt-0 sm:pt-0">
              ...
              <div className="border-t border-border pt-3"><p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Recent evidence events</p><div className="space-y-2">{contract.events.slice(0, 12).map((event) => <div key={event.id} className="flex items-start justify-between gap-3 text-xs"><span className="font-medium capitalize">{event.eventType.replaceAll("_", " ")}</span><span className="shrink-0 text-[10px] text-muted-foreground">{new Date(event.createdAt).toLocaleString()}</span></div>)}</div></div>
            </CardContent>
          </Card>
        </aside>
      </div>
      ...
      <Dialog open={voidDialogOpen} onOpenChange={(open) => { if (busy !== "void") setVoidDialogOpen(open); }}>
        <DialogContent>
          ...
          <DialogTitle className="mt-3 text-xl font-extrabold">Void this contract record?</DialogTitle>
          <DialogDescription className="mt-2 leading-6">Active review and signing links will be revoked. The contract, versions, comments, and evidence stay retained for history.</DialogDescription>
          <div className="mt-5 flex justify-end gap-2"><Button variant="outline" onClick={() => setVoidDialogOpen(false)}>Cancel</Button><Button variant="destructive" disabled={busy === "void"} onClick={async () => { const result = await runAction("void", `/api/workflow/contracts/${id}`, "DELETE"); if (result) setVoidDialogOpen(false); }}>{busy === "void" ? <Loader2 className="h-4 w-4 animate-spin" /> : <XCircle className="h-4 w-4" />} Void contract</Button></div>
        </DialogContent>
      </Dialog>
    </div>
  );
...
function ContractProgress({ status }: { status: string }) {
  ...
  const stages = ["Draft", "Client review", "Signatures", "Executed"];
  return <ol className="grid grid-cols-4 gap-2" aria-label="Contract progress">{stages.map((stage, index) => <li key={stage} className="min-w-0"><div className={`h-1.5 rounded-full ${index <= current ? status === "void" || status === "declined" ? "bg-amber-500" : "bg-primary" : "bg-muted"}`} /><p className={`mt-1.5 truncate text-[10px] font-bold sm:text-xs ${index === current ? "text-foreground" : "text-muted-foreground"}`}>{stage}</p></li>)}</ol>;
}
...
function NextActionCard({ contract, versionApproved, openComments, canReview, canFinalize, busy, onAction, onFinalize }: { contract: Contract; versionApproved: boolean; openComments: number; canReview: boolean; canFinalize: boolean; busy: string | null; onAction: (key: string, url: string, method?: string, body?: unknown, preserveEditor?: boolean) => Promise<Record<string, unknown> | null>; onFinalize: () => void }) {
  ...
  if (contract.status === "in_review") {
    ...
    description = versionApproved ? "Finalize this exact version when you are satisfied, then issue a separate signing request." : openComments ? "Resolve the negotiation thread or edit a new immutable version before finalizing." : "You can resend a fresh review link or finalize after confirming the client is ready.";
  } else if (contract.status === "ready_to_sign") {
    title = "Start the signature request";
    description = "The client signs first. Your owner link unlocks after their signature is recorded.";
  } else if (contract.status === "signing") {
    title = client?.status !== "signed" ? "Waiting for the client signature" : owner?.status !== "signed" ? "Your signature is next" : "Completing execution";
    description = "Fresh links can be issued from the signing-party cards. Old links are revoked automatically.";
  } else if (contract.status === "declined") {
    title = "A signer requested changes";
    description = contract.signers.some((signer) => signer.signatures.length) ? "Because a signature already exists, void this record and create a replacement agreement." : "Read the decline reason in the evidence timeline, revise the draft, and run review again.";
  } else if (contract.status === "executed") {
    title = "Contract executed — review billing drafts";
