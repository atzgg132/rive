          ...
          {contract.void_requested_at ? (
            contract.void_requested_by_role === "client" ? (
              <>
                ...
                <p className="mt-2 text-xs">Confirming voids the Agreement for both parties: unsent invoice drafts from it are cancelled and no further ones are drafted. Sent invoices are unchanged.</p>
            ...
            <>
              ...
              <p className="mt-1 text-xs">Both parties must agree. The client is emailed a link to confirm; the record is retained either way. Once voided, invoices from this Agreement that were never sent are cancelled and no further ones are drafted; sent invoices are unchanged.</p>
...
function NextActionCard({ contract, clientAcceptedAt, versionStatus, versionApproved, openComments, canFinalize, canOwnerAccept, canRestartAcceptance, busy, onAction, onFinalize, onAcceptOwn, onRestart, onEdit }: { contract: Contract; clientAcceptedAt: string | null; versionStatus: string | null; versionApproved: boolean; openComments: number; canFinalize: boolean; canOwnerAccept: boolean; canRestartAcceptance: boolean; busy: string | null; onAction: RunAction; onFinalize: () => void; onAcceptOwn: () => void; onRestart: () => void; onEdit: () => void }) {
  ...
  if (contract.status === "in_review") {
    ...
    const drafted = contract.payment_plan.filter((item) => item.occurrence?.invoice).length;
    title = "Agreement accepted — the payment plan is active";
    description = contract.payment_plan.length === 0
      ? "There is no payment plan, so billing stays manual."
      : `Invoices draft automatically as each payment comes due (${drafted} of ${contract.payment_plan.length} drafted so far). Review each draft before sending it.${workReady ? "" : " Setting up the work below is optional planning."}`;
    actions = <><Link href="/workflow/revenue" className={buttonVariants({ variant: "default" })}><CircleDollarSign className="h-4 w-4" /> Review invoices</Link>{workReady ? null : <Link href="#work-setup" className={buttonVariants({ variant: "outline" })}><ArrowRight className="h-4 w-4" /> Set up the work</Link>}{contract.payment_plan.length ? <Button variant="outline" disabled={Boolean(busy)} onClick={() => void onAction("billing", `/api/workflow/contracts/${contract.id}/billing/run`)}><RefreshCw className="h-4 w-4" /> Check triggers</Button> : null}</>;
