              ...
              return (
                <DroppableColumn key={col.id} id={col.id} title={col.title} color={col.color} count={filteredProjects.length}>
                  {filteredProjects.length === 0 ? (
                    <div className="text-center py-8 text-xs text-muted-foreground dark:text-slate-500 border border-dashed border-border dark:border-slate-700 rounded-xl bg-white/30 dark:bg-slate-900/30">
    ...
    <div ref={setNodeRef} style={style} className="group relative flex flex-col gap-4 rounded-xl border border-border bg-card p-5 shadow-sm transition-[border-color,box-shadow] hover:border-primary/25 hover:shadow-card">
      ...
      <div>
        <div className="flex justify-between items-start gap-3 mb-2 pr-6">
          ...
          <span className={`shrink-0 rounded-md border px-2 py-1 text-xs font-semibold capitalize ${getPriorityColor(project.priority)}`}>
            {project.priority}
          </span>
        </div>
          ...
          {budgetAmount !== null && (
            <span className="flex flex-col items-end text-right font-extrabold text-[#10B981] dark:text-emerald-400">
              ...
              {project.currency !== displayCurrency && convertedBudget && (
                <span className="text-xs font-medium text-muted-foreground">Originally {format(budgetAmount, project.currency)}</span>
              )}
            </span>
          )}
        </div>
      ...
      {agreements && <div className="ml-6 flex items-center justify-between gap-3 rounded-lg bg-muted/50 px-3 py-2 text-xs">
