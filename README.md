...
Production runs on AWS in `ap-south-1`. The `dev` and `main` branches deploy to isolated AWS environments through GitHub Actions after the quality gate passes. Work lands on `dev` via typed branch PRs (`feature/`, `fix/`, `chore/`, `docs/` → squash merge) and promotes to `main` in batches via merge-commit PR — the full policy lives in [`AGENTS.md`](AGENTS.md#shipping). Infrastructure, secrets, migrations, and deployment details live in [`infrastructure/README.md`](infrastructure/README.md).
